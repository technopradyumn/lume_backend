...
const getUserTweets = asyncHandler(async (req: Request, res: Response) => {
  ...
  const tweets = await Tweet.find({ owner: userId as any })
  ...
  const tweetsWithLikes = await Promise.all(
    tweets.map(async (t) => {
      ...
      if (req.user) {
        ...
        isLiked = !!userLike;
        if ((t.owner as any)?._id) {
          const sub = await Subscription.findOne({
            subscriber: req.user._id,
            channel: (t.owner as any)._id,
          });
          isSubscribed = !!sub;
        }
      }
...
const getTweets = asyncHandler(async (req: Request, res: Response) => {
...
const addTweetReply = asyncHandler(async (req: Request, res: Response) => {
  const { tweetId } = req.params;
  const { content } = req.body;
